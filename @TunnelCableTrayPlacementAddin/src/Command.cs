        ...
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            try
            {
                ...
                if (uiDocument == null || uiDocument.Document == null)
                {
                    ...
                    TaskDialog.Show("터널 케이블 트레이 자동배치", message);
                    return Result.Cancelled;
                }

                string addinFolder = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location);
                string enginePath = Path.Combine(addinFolder, "TunnelCableTrayPlacementEngine.dll");
                ...
                Type entryType = engineAssembly.GetType("TunnelCableTrayPlacementAddin.EngineEntry");
            ...
            catch (TargetInvocationException ex)
            {
                ...
                TaskDialog.Show("터널 케이블 트레이 자동배치 - 오류", inner.ToString());
                return Result.Cancelled;
            }
            catch (Exception ex)
            {
                message = ex.Message;
                TaskDialog.Show("터널 케이블 트레이 자동배치 - 오류", ex.ToString());
                return Result.Cancelled;
            }
        }
    }
