...
def find_face_in_link_from_reference(link_instance, reference):
    ...
    if link_reference is not None:
        ...
        if isinstance(geometry_object, Face):
            best_face = geometry_object
            best_transform = link_transform
            return best_linked_element, best_face, best_transform
...
def pick_face_reference():
    ...
    try:
        reference = uidoc.Selection.PickObject(
            ...
            "Select one Revit Link element for Z-only attachment",
        )
        return reference, None
