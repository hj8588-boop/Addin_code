...
def get_preferred_target_point(
    ...
    face_sources = source_bottom_points or [source_point]
    ...
    if precise_selection or auto_edge_selection:
        ...
        if face_candidates:
            try:
                highest_z = max([candidate.Z for candidate in face_candidates])
                return XYZ(source_point.X, source_point.Y, highest_z), max_xy_distance, "faceAtSourceXY"
            except Exception:
...
def find_face_in_import_from_reference(element, reference, source_points=None):
    ...
    if isinstance(geometry_face, Face):
        return geometry_face, None, "directReference"
...
def main():
        ...
        if selection_mode == "link:pickedPointOnly" and picked_point is not None:
            ...
            try:
                for family in families:
                    ...
                    move_delta_x = picked_point.X - attachment_anchor_point.X
                    move_delta_y = picked_point.Y - attachment_anchor_point.Y
                    target_point = XYZ(picked_point.X, picked_point.Y, picked_point.Z)
                    vertical_delta = target_point.Z - attachment_anchor_point.Z
                    xy_distance = ((move_delta_x * move_delta_x) + (move_delta_y * move_delta_y)) ** 0.5

                    move_distance_sq = (move_delta_x * move_delta_x) + (move_delta_y * move_delta_y) + (vertical_delta * vertical_delta)
                    ...
                    try:
                        ...
                        ElementTransformUtils.MoveElement(
                            doc,
                            family.Id,
                            XYZ(move_delta_x, move_delta_y, vertical_delta),
                        )
                    ...
                    move_details.append(
                        {
                            ...
                            "xyDistanceMm": round(from_internal_mm(xy_distance), 2),
                            "movedDeltaXMm": round(from_internal_mm(move_delta_x), 2),
                            "movedDeltaYMm": round(from_internal_mm(move_delta_y), 2),
                            "movedDeltaZMm": round(from_internal_mm(vertical_delta), 2),
                            "targetMode": "pickedPointZAbsolute",
                            "status": "movedToPickedPoint",
                        }
                    )
                ...
                return make_result(
                    ...
                    message="Completed. The selected elements were moved in Z to the picked point height.",
                )
            finally:
    ...
    try:
        ...
        for family in families:
            ...
            move_delta_x = 0.0
            move_delta_y = 0.0
            evaluation_source_point = source_point
            evaluation_bottom_points = get_instance_bottom_corners(family)
            evaluation_anchor_point = attachment_anchor_point

            if should_move_to_picked_xy(selection_mode, picked_point):
                try:
                    move_delta_x = picked_point.X - attachment_anchor_point.X
                    move_delta_y = picked_point.Y - attachment_anchor_point.Y
                    evaluation_source_point = move_point_xy(
                        attachment_anchor_point,
                        move_delta_x,
                        move_delta_y,
                    )
                    evaluation_anchor_point = evaluation_source_point
                    evaluation_bottom_points = [evaluation_anchor_point]
                except Exception:
                    move_delta_x = 0.0
                    move_delta_y = 0.0
            ...
            vertical_delta = target_point.Z - evaluation_anchor_point.Z
            move_distance_sq = (move_delta_x * move_delta_x) + (move_delta_y * move_delta_y) + (vertical_delta * vertical_delta)
            if move_distance_sq < (min_vertical_gap_internal * min_vertical_gap_internal):
                skipped.append(
                    {
    ...
    return make_result(
        ...
        skipped=skipped,
        selected_face_summary=selected_face_summary,
        message="Completed. The selected elements were moved in Z to the picked face.",
        traced_model_curve_ids=traced_model_curve_ids,
        traced_sketch_plane_id=traced_sketch_plane_id,
        helper_directshape_id=helper_directshape_id,
    )
