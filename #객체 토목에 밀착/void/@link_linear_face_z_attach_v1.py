...
allow_link_picked_point_fallback = False
...
def get_preferred_target_point(
    ...
    if (
        allow_link_picked_point_fallback
        and picked_point is not None
...
def pick_face_reference():
    ...
    try:
        reference = uidoc.Selection.PickObject(
            ...
            "Select one Revit face or Revit Link planar face for linear Z attachment",
        )
    ...
    try:
        reference = uidoc.Selection.PickObject(
            ...
            "Select one Revit Link element for linear planar-face attachment if direct face picking is not available",
        )
...
def get_face_from_reference(reference, pick_kind=None, source_points=None):
    ...
    if isinstance(element, RevitLinkInstance):
        ...
        if allow_link_picked_point_fallback and global_point is not None:
...
def main():
    ...
    if isinstance(host_element, RevitLinkInstance) and selected_face is not None and not isinstance(selected_face, PlanarFace):
        return make_result(
            selected_family_count=len(families),
            message="The picked Revit Link reference is not a usable planar face. Pick a clearer linear planar face and try again.",
        )
    if selected_face is None:
        if allow_link_picked_point_fallback and selection_mode == "link:pickedPointOnly" and picked_point is not None:
        ...
        return make_result(
            selected_family_count=len(families),
            message="The picked reference did not resolve to a usable planar face. Pick a clearer link/revit planar face and try again.",
        )
