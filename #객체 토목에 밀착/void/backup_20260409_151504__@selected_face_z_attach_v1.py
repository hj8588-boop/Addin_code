...
def is_precise_selection_mode(selection_mode):
    ...
    if str(selection_mode).startswith("helperDirectShape:"):
        return True
...
def create_helper_directshape_from_planar_face(face, transform=None):
    if not isinstance(face, PlanarFace):
        return None, [], "The selected face is not a PlanarFace."

    curve_loops = build_curve_loops_from_planar_face(face, transform)
    if not curve_loops:
        return None, [], "No valid curve loops could be built from the planar face."
    ...
    try:
        ...
        return None, [], helper_error or "Triangulate failed: {0}".format(mesh_error)

    if mesh is None:
        return None, [], helper_error or "Triangulate returned null."

    try:
        _, plane_normal = get_world_plane_from_planar_face(face, transform)
        ...
        for index in range(triangle_count):
            ...
            triangle_points_raw = [
                apply_transform_to_point(triangle.get_Vertex(0), transform),
                apply_transform_to_point(triangle.get_Vertex(1), transform),
                apply_transform_to_point(triangle.get_Vertex(2), transform),
            ]
            triangle_points_raw = orient_triangle_points(triangle_points_raw, plane_normal)
            triangle_points = List[XYZ]()
            triangle_points.Add(triangle_points_raw[0])
            triangle_points.Add(triangle_points_raw[1])
            triangle_points.Add(triangle_points_raw[2])
        ...
        geometry_result = build_result.GetGeometricalObjects()
        direct_shape.SetShape(geometry_result)
    except Exception as tessellation_error:
        return None, [], helper_error or "TessellatedShapeBuilder failed: {0}".format(tessellation_error)
    ...
    for face, face_transform in iter_faces_from_geometry(geometry_element, base_transform):
        ...
        if picked_point is not None:
            ...
            if isinstance(face, PlanarFace):
                ...
                picked_z_gap = abs(picked_planar_point.Z - picked_point.Z)
                if picked_z_gap > to_internal_mm(planar_pick_z_tolerance_mm):
                    continue
...
def main():
    ...
    try:
        if (
            ...
            helper_directshape, helper_curve_loops, helper_directshape_error = create_helper_directshape_from_planar_face(
                selected_face,
                selected_face_transform,
            )
            helper_directshape_id = safe_int_id(helper_directshape)
            ...
            if helper_directshape_id is not None:
                selected_face_summary["helperDirectShapeId"] = helper_directshape_id
                selected_face_summary["helperCurveLoopCount"] = len(helper_curve_loops)
                helper_face, helper_transform, helper_mode = find_best_face_in_element_for_sources(
                    helper_directshape,
                    family_source_points,
                    picked_point,
                )
                if helper_face is not None:
                    host_element = helper_directshape
                    selected_face = helper_face
                    selected_face_transform = helper_transform
                    selection_mode = "helperDirectShape:" + str(helper_mode)
                    selected_face_summary["hostId"] = safe_int_id(host_element)
                    selected_face_summary["hostType"] = type(host_element).__name__
                    selected_face_summary["selectionMode"] = selection_mode
            elif helper_directshape_error is not None:
                selected_face_summary["helperDirectShapeError"] = helper_directshape_error
