...
ROOM_TARGET_FIXTURE_PARAM = "조도_적용전등타입"
FIXTURE_FLUX_PARAM = "fixtureFlux_lm"
ROOM_INPUT_FLUX_PARAM_NAMES = [
    "조도_광속_lm",
    FIXTURE_FLUX_PARAM,
    "광속",
    "광속(lm)",
]
FIXED_CEILING_REFLECTANCE = 70.0
FIXED_WALL_REFLECTANCE = 50.0
FIXED_FLOOR_REFLECTANCE = 10.0
...
def get_all_spaces():
    if Space is None:
        return []
    elements = list(
        FilteredElementCollector(doc).OfCategory(BuiltInCategory.OST_MEPSpaces).WhereElementIsNotElementType()
    )
    return [element for element in elements if is_room_or_space(element)]
...
def is_room_or_space(element):
    ...
    try:
        ...
        return False
    return category_id == int(BuiltInCategory.OST_MEPSpaces)
...
def get_fixture_type_name(fixture_instance):
    fixture_type = get_fixture_type(fixture_instance)
    try:
        return fixture_type.get_Parameter(BuiltInParameter.SYMBOL_NAME_PARAM).AsString()
    except Exception:
        try:
            return fixture_instance.Name
        except Exception:
            return None
...
def build_fixture_type_lookup():
    fixture_map = {}
    all_fixtures = FilteredElementCollector(doc).OfCategory(BuiltInCategory.OST_LightingFixtures).WhereElementIsNotElementType()

    for fixture in all_fixtures:
        if not is_lighting_fixture(fixture):
            continue

        fixture_label = get_fixture_label(fixture)
        fixture_type_name = get_fixture_type_name(fixture)
        fixture_type = get_fixture_type(fixture)
        family_name = None
        try:
            family_name = fixture_type.FamilyName
        except Exception:
        ...
        keys = [
            normalize_lookup_text(fixture_label),
            normalize_lookup_text(fixture_type_name),
            normalize_lookup_text(family_name),
        ]
        if family_name and fixture_type_name:
            keys.append(normalize_lookup_text("{0} : {1}".format(family_name, fixture_type_name)))
            keys.append(normalize_lookup_text("{0}:{1}".format(family_name, fixture_type_name)))

        for key in keys:
            if key and key not in fixture_map:
...
def read_room_input_flux(room_or_space):
    return read_numeric_parameter(room_or_space, ROOM_INPUT_FLUX_PARAM_NAMES)
...
def read_reflectance_parameter(room_or_space):
    return FIXED_WALL_REFLECTANCE, "fixed"


def read_surface_reflectance_parameters(room_or_space):
    return {
        "wallReflectance": FIXED_WALL_REFLECTANCE,
        "wallReflectanceParam": "fixed",
        "ceilingReflectance": FIXED_CEILING_REFLECTANCE,
        "ceilingReflectanceParam": "fixed",
        "floorReflectance": FIXED_FLOOR_REFLECTANCE,
        "floorReflectanceParam": "fixed",
    }
...
def normalize_space_name_for_filter(value):
    text = safe_string(value).strip()
    return text.lower() if text else ""


def has_meaningful_room_name(room_or_space):
    room_name_value, _ = get_room_name_parameter_value(room_or_space)
    normalized_name = normalize_space_name_for_filter(room_name_value)
    if not normalized_name:
        return False
    if normalized_name in ["space", "room"]:
        return False
    if normalized_name == "unoccupied":
        return False
    if normalized_name.startswith("space "):
        return False
    if normalized_name.startswith("room "):
        return False
    if normalized_name.startswith("unoccupied "):
        return False
    return True
...
def get_room_level_id(room_or_space):
    try:
        level_id = room_or_space.LevelId
        if level_id and level_id != ElementId.InvalidElementId:
            return level_id
...
def get_target_fixture_value(room_or_space):
    return read_string_parameter(
        room_or_space,
        [ROOM_TARGET_FIXTURE_PARAM, "적용전등타입", "적용 전등 타입", "fixture type", "Fixture Type"],
    )


def get_element_category_name(element):
    try:
        return safe_string(element.Category.Name).strip()
    except Exception:
        return None
            ...
            with open(file_path, "w", newline="", encoding="utf-8-sig") as csv_file:
                ...
                for room_result in room_results:
                    ...
                    row_values = {
                        ...
                        "fixture": room_result.get("fixture"),
...
if not RUN_GRAPH:
    ...
    selected_seed_rooms = [element for element in selected_elements if is_room_or_space(element)]

    if not selected_seed_rooms:
        OUT = {
            "status": "error",
        ...
        target_level_ids = []
        for seed_room in selected_seed_rooms:
            level_id = get_room_level_id(seed_room)
            if level_id is not None and level_id not in target_level_ids:
                target_level_ids.append(level_id)

        if not target_level_ids:
            OUT = {
                "status": "error",
                "message": "선택한 Lighting Fixture에서 광속 값을 읽지 못했습니다.",
                "selectedRooms": len(selected_seed_rooms),
            }
        else:
            selected_rooms = [
                room for room in get_all_spaces()
                if get_room_level_id(room) in target_level_ids and has_meaningful_room_name(room)
            ]
            fixture_lookup = build_fixture_type_lookup()
            room_results = []
            writable_rooms = []

            for room in selected_rooms:
                target_fixture_value, target_fixture_param = get_target_fixture_value(room)
                target_fixture_key = normalize_lookup_text(target_fixture_value)
                fixture = fixture_lookup.get(target_fixture_key)
                room_input_flux, room_input_flux_param = read_room_input_flux(room)
                fixture_flux, fixture_flux_param_name = read_fixture_flux(fixture) if fixture is not None else (None, None)
                if room_input_flux is not None and room_input_flux > 0:
                    fixture_flux = room_input_flux
                    fixture_flux_param_name = room_input_flux_param

                fixture_label = get_fixture_label(fixture) if fixture is not None else None
                if not fixture_label and target_fixture_value:
                    fixture_label = target_fixture_value
                ...
                room_result = {
                    ...
                    "category": get_element_category_name(room),
                    "room": get_room_label(room),
                    "roomNameParamValue": room_name_value,
                    "levelName": level_name,
                    "targetFixtureType": target_fixture_value,
                ...
                if not target_fixture_value:
                    missing.append(ROOM_TARGET_FIXTURE_PARAM)
                elif fixture is None and not (room_input_flux is not None and room_input_flux > 0):
                    missing.append("{0} 매칭 실패".format(target_fixture_value))
                if fixture_flux is None or fixture_flux <= 0:
                    missing.append("조도_광속_lm 또는 fixtureFlux_lm")
                ...
                room_result["message"] = "반사율 고정값 적용: 천장 70 / 벽 50 / 바닥 10"
                ...
                room_result["usedParameters"] = {
                    ...
                    "targetFixtureType": target_fixture_param,
                }
                room_results.append(room_result)
            ...
            export_path, export_error = export_room_results_to_csv(doc, None, room_results)

            OUT = {
                "status": "ok",
                "message": "선택한 Lighting Fixture 기준으로 Room/Space 필요 조명 개수를 계산했습니다.",
                "selectedRoomCount": len(selected_rooms),
                "selectedSeedRoomCount": len(selected_seed_rooms),
                "targetLevelCount": len(target_level_ids),
                "formula": "CEILING((요구조도 * 면적) / (광속 * 조명율 * 보수율))",
                "note": "반사율은 고정값(천장 70 / 벽 50 / 바닥 10)으로 기록되며, 현재 계산식에는 직접 사용하지 않습니다.",
